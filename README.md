...
- **Кнопка «Развернуть / Свернуть» для просмотра фотографий** — в PhotoCard добавлен лайтбокс: кнопка «Развернуть» (top-right, появляется при наведении) открывает фото поверх страницы на тёмном фоне; кнопка «Свернуть», клик по фону и `Escape` закрывают оверлей. Навигация стрелками и свайп сохраняются. Кнопка «Original» (зажать) работает внутри лайтбокса. Бейдж «★ AI Pick» отображается и в обычном виде, и в оверлее. Переводы: RU «Развернуть / Свернуть», EN «Expand / Collapse»
