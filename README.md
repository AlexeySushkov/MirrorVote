# MirrorVote — AI‑помощник выбора образа
...
- [**Приложение в Internet**](https://mirror-vote.ru/app)
- [**Лендинг в Internet**](https://mirror-vote.ru/)
...
1. **Владелец сессии** — на странице «Мои примерки» нажимает кнопку «Ссылка для голосования» в карточке сессии. При первом нажатии генерируется уникальный `share_token`, затем открывается диалог с выбором действия: скопировать ссылку в буфер или открыть в новом окне браузера (например, `https://app.com/v/abc123def456`).
...
### Безопасность ссылки для голосования

Ссылка намеренно публичная — владелец сам решает, кому её передать. Ниже описаны реализованные уровни защиты.

#### Уровни защиты

| Угроза | Механизм защиты | Статус |
|--------|----------------|--------|
| Перебор токенов | 12 hex-символов из `crypto.randomUUID()` = 48 бит энтропии = ~281 триллион вариантов | ✅ Надёжно |
| Прямой SELECT чужих сессий | Row Level Security: `USING (auth.uid() = user_id)` на всех таблицах | ✅ Заблокировано |
| Прямая запись голосов в БД | Политика `FOR ALL USING (false)` на `mirror_votes` — таблица закрыта полностью | ✅ Заблокировано |
| Накрутка оценки 1–5 | Валидация диапазона в `submit_rating`: `RAISE EXCEPTION` при rating < 1 или > 5 | ✅ Заблокировано |
| Утечка приватных полей через ссылку | `get_public_session` возвращает только `id`, `title`, `background` сессии и нужные поля фото — без `user_id`, `ai_recommendation`, `share_token` и т.д. (миграция `010`) | ✅ Исправлено |
| Накрутка голосов с одного устройства | Fingerprint в localStorage (`mv_fp_{token}`); `UNIQUE(session_id, photo_id, fingerprint)` в БД | ⚠️ Мягкая защита |

#### Что видит голосующий по ссылке

По ссылке `https://mirror-vote.ru/app/v/{token}` доступно только:
- название сессии и тип случая (background)
- фотографии нарядов (URL из публичного bucket `mirror_photos`)
- средний рейтинг и количество оценок по каждому фото

**Не передаётся:** `user_id` владельца, AI-рекомендации, статус сессии, токен, любые внутренние идентификаторы.

#### Ограничения

- **Накрутка голосов:** очистка `localStorage` позволяет проголосовать повторно. Fingerprint — защита от случайного двойного голоса, но не от намеренной накрутки. Для критичных голосований рассмотреть IP-ограничение или авторизацию.
- **Фото публичны:** bucket `mirror_photos` — public. Любой, у кого есть прямой URL фото, может его загрузить, даже без токена сессии. Для приватности — перейти на signed URLs (см. TODO).
- **Ссылка бессрочна:** токен не имеет срока действия. Отозвать ссылку можно только удалив `share_token` из сессии вручную в БД.
...
- [x] **Свайп-навигация** — жесты влево/вправо на мобильных (все режимы, включая полноэкранный в карусели)
- [x] **Полноэкранный режим** — просмотр фотографий на весь экран
...
- **Версия `1.0.5`** — `package.json` обновлён до версии 1.0.5
- **Версия в шапке** — в дропдауне пользователя (после «Выход», с разделителем) отображается номер версии и дата сборки (`v1.0.5 · 31.05.2026`). Константы `__APP_VERSION__` и `__BUILD_DATE__` задаются в `vite.config.ts` через `define`; типы объявлены в `src/vite-env.d.ts`
- **«AI оценка:» перед оценкой** — в `InlineVerdict` добавлен лейбл «AI оценка:» рядом с числовой оценкой фото
- **Кнопки навигации перенесены наверх в Compare** — кнопки «Мои примерки» и «Добавить фото» теперь располагаются первой группой (до кнопок AI-оценки). Кнопки «Экспорт коллажа» и «Оригинал» временно скрыты
- **Обнаружение ошибок сети** — новая функция `isNetworkError()` в `errorToast.ts` (проверяет `navigator.onLine` и паттерны `message`). При сетевой ошибке в Compare и VotePage показывается «Нет подключения к интернету» вместо общей ошибки. Переводы `error.noInternet` добавлены в `LanguageContext` (RU/EN)
- **Фикс «Показать все» в карусели** — кнопка «Показать все» теперь появляется только если в сессии изначально больше одного фото (раньше показывалась при одном скрытом фото даже в сессии с одним фото)
- **Чекбокс без скругления** — `checkbox.tsx`: `rounded-sm` → `rounded-none`

### 2026-05-27

- **Иконка приложения** — новый дизайн favicon: оранжево-красный градиент, белое сердце с галочкой, золотые искры. Перегенерированы все PWA PNG (192, 512, maskable, apple-touch-icon)
- **Форма входа** — обновлён заголовок «AI-помощник в примерочной» → «AI‑помощник выбора образа» (ru/en); добавлена эмблема приложения над заголовком карточки
- **Диалог ссылки для голосования** — после нажатия «Ссылка для голосования» на странице Compare открывается диалог с тремя кнопками: «Скопировать ссылку» (копирует в буфер, закрывает диалог), «Открыть в новом окне» (открывает вкладку, закрывает диалог), «Отмена». Кнопка убрана из карточек списка сессий
- **Только карусель** — убрана вкладка «Pick Best» со страницы Compare; `CarouselView` — единственный и дефолтный режим просмотра, тулбар с вкладками удалён
- **Кнопка «Удалить выбранное»** — первая кнопка в шапке страницы сессий; появляется только при наличии выбранных карточек
- **Скрытие фото в карусели** — на каждом фото кнопка «Скрыть» (нижний левый угол, рядом с Original); при нажатии фото исчезает из карусели. Когда остаётся одно фото, кнопка меняется на «Показать все» — восстанавливает все скрытые
- **Статусы сессий** — переименованы: Загрузка → «Новая», Обработка → «Обработка AI», Готово → «Обработано AI», Проанализировано → «Оценено AI»
- **Кнопка голосования** — «Попробовать MirrorVote» → «Перейти в MirrorVote» на публичной странице голосования
- **Форма выбора случая** — кнопка «Оценить» стала белой (outline), кнопка «Отмена» — розовой (destructive)
- **Фикс безопасности: утечка данных в `get_public_session`** — функция возвращала `to_jsonb(s)` — полную строку `mirror_sessions`, включая `user_id`, `ai_recommendation`, `best_photo_id`, `share_token`. Любой с токеном голосования видел эти поля в DevTools. Заменено на явный `jsonb_build_object` с тремя полями (`id`, `title`, `background`). Миграция: `010_fix_public_session_data_leak.sql`

### 2026-05-26

- **Фикс: стрелки и свайп в полноэкранном режиме карусели** — при открытии лайтбокса из карусели стрелки и свайп не реагировали. Причина: у каждого фото в карусели свой экземпляр `PhotoCard` с зафиксированным `photo`-пропом; стрелки скроллили Embla, но оверлей продолжал показывать то же фото. Исправлено: полноэкранный оверлей перенесён в `CarouselView`, привязан к `photos[selectedIndex]` и обновляется при каждом прокруте карусели. `PhotoCard` получил опциональный проп `onExpand` — при наличии делегирует открытие лайтбокса родителю; лайтбокс внутри `PhotoCard` сохранён для `PickBestView`/`BeforeAfterView`.
...
- **Кнопка «Развернуть / Свернуть» для просмотра фотографий** — в PhotoCard добавлен лайтбокс: кнопка «Развернуть» (top-right, всегда видна) открывает фото поверх страницы на тёмном фоне; кнопка «Свернуть», клик по фону и `Escape` закрывают оверлей. Навигация стрелками и свайп сохраняются. Кнопка «Original» (зажать) работает внутри лайтбокса. Бейдж «★ AI Pick» отображается и в обычном виде, и в оверлее. Переводы: RU «Развернуть / Свернуть», EN «Expand / Collapse»
- **Фикс: кнопка «Развернуть» теперь всегда видна** — убрано `opacity-0 group-hover:opacity-100`; видимость одинакова с кнопкой «Original»
- **Фикс кнопки «Обновить» в PWA** — кнопка была видна, но нажатие ничего не делало. Причина: `clientsClaim: false` в workbox-конфиге не позволял новому SW взять контроль над страницей, поэтому событие `controllerchange` никогда не срабатывало и `updateServiceWorker(true)` не мог вызвать `window.location.reload()`. Исправлено: `clientsClaim: true` + fallback-таймер на 1 с в onClick, на случай задержки SW. Также добавлен `padding-bottom: env(safe-area-inset-bottom)` для корректного отображения баннера над жестовой навигацией Android/iOS
...
- **Кнопка «AI Look»** — кнопка переключения «Вернуть Look» / «Back to Look» переименована в «AI Look» для краткости и единообразия с остальными лейблами
- **Адаптивная шапка страницы сессий** — на мобильных кнопки заголовка перестраиваются в две строки (название + счётчик / кнопки действий), на десктопе — одна строка
- **Мобильные превью фото в карточке сессии** — полоса миниатюр вынесена в отдельную строку на всю ширину карточки; текст, бейдж и кнопка «Скопировать ссылку» теперь располагаются под превью и больше не конкурируют за горизонтальное пространство
...
- **Цвет кнопки destructive совпадает с favicon** — изменён оттенок с тёплого красно-оранжевого (`8 75% 48%`) на розово-красный (`344 55% 53%`), соответствующий тёмному концу градиента favicon (`#C94668`). Тёмная тема обновлена аналогично (`344 45% 38%`)
- **Снижена цена по умолчанию пакета 5 оценок** — дефолтное значение `YOOKASSA_PACK_5_AMOUNT` изменено с `99.00` на `50.00`
